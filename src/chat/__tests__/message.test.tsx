...
describe('Test Chat Message', () => {
    ...
    it('Should render pagination since multiple messages', () => {
        ...
        expect(container.querySelector('.dt-aigc-pagination')).not.toBeNull();
        expect(container.querySelector('.dt-aigc-pagination section')?.textContent).toBe('2/2');
        expect(container.querySelector('.dtc__aigc__markdown')?.textContent).toBe(
            'My Name is dt-design2'
        );

        act(() => {
            fireEvent.click(container.querySelector('.dt-aigc-pagination')!.firstElementChild!);
        });

        expect(container.querySelector('.dtc__aigc__markdown')?.textContent).toBe(
            'My Name is dt-design'
        );
        expect(container.querySelector('.dt-aigc-pagination section')?.textContent).toBe('1/2');
    });
    ...
    it('Should support configure copy', async () => {
        ...
        const classNames = '.dtc__message__icon.dt-copy';
        const ele = container.querySelector(classNames);
        expect(ele).not.toBeNull();
        act(() => {
            fireEvent.click(ele!);
        });
        expect(onCopy).toBeCalledWith('My Name is dt-design');
        ...
        expect(onCopy).toBeCalledWith('My Name is dt-design jest');
    });
