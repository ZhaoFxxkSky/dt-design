...
class Collapsible extends React.Component<CollapsibleProps, CollapsibleState> {
  ...
  componentDidUpdate(prevProps: Readonly<CollapsibleProps>, prevState: Readonly<CollapsibleState>) {
  ...
  renderCollapsible = ({ getPrefixCls }: ConfigConsumerProps) => {
    const {
      isOpen,
      collapseHeight,
      fade,
  ...
  render() {
    return <ConfigConsumer>{this.renderCollapsible}</ConfigConsumer>;
  }
