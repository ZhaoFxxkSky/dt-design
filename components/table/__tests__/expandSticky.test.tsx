...
describe('useExpand', () => {
  ...
  it('toggles expansion on trigger', () => {
    ...
    act(() => {
      onTriggerExpand(data[0], null as any);
    });
  ...
  it('returns "nest" type when data has children but no expandedRowRender', () => {
    ...
    const { result } = renderHook(() => useExpand({ data } as any, data, (r: any) => r.key));
  ...
  it('calls onExpand when toggling', () => {
    ...
    act(() => {
      result.current[5](data[0], null as any);
    });
  ...
  it('renders default expand icon', () => {
    ...
    expect(container.querySelector('.ant-table-row-expanded')).toBeInTheDocument();
  });
  ...
  it('renders collapsed expand icon', () => {
    ...
    expect(container.querySelector('.ant-table-row-collapsed')).toBeInTheDocument();
  });
...
describe('useSticky', () => {
  ...
  it('returns isSticky=true with offsets when sticky is an object', () => {
    ...
    expect(result.current.stickyClassName).toContain('sticky-holder');
  });
...
describe('useFixedInfo', () => {
  it('returns fixed info for start-fixed columns', () => {
    ...
    const { result } = renderHook(() => useFixedInfo(columns, stickyOffsets as any));
    expect(result.current[0].fixStart).toBe(0);
    expect(result.current[1].fixStart).toBe(100);
    expect(result.current[2].fixStart).toBe(null);
    expect(result.current[3].fixEnd).toBe(0);
  });
  ...
  it('returns empty info for non-fixed columns', () => {
    ...
    const { result } = renderHook(() => useFixedInfo(columns, stickyOffsets as any));
    expect(result.current[0].fixStart).toBe(null);
    expect(result.current[0].fixEnd).toBe(null);
    expect(result.current[1].fixStart).toBe(null);
    expect(result.current[1].fixEnd).toBe(null);
  });
  ...
  it('handles "left" as fixed (maps to start)', () => {
    ...
    const { result } = renderHook(() => useFixedInfo(columns, stickyOffsets as any));
    // 'left' is converted to 'start' by flatColumns, not useFixedInfo
    expect(result.current[0].fixStart).toBe(null);
  });
  ...
  it('handles "right" as fixed (maps to end)', () => {
    ...
    const stickyOffsets = { start: [0], end: [0], widths: [100] };
    const { result } = renderHook(() => useFixedInfo(columns, stickyOffsets as any));
    // 'right' is converted to 'end' by flatColumns, not useFixedInfo
    expect(result.current[0].fixEnd).toBe(null);
  });
  ...
  it('handles boolean true as fixed (maps to start)', () => {
    ...
    const stickyOffsets = { start: [0], end: [0], widths: [100] };
    const { result } = renderHook(() => useFixedInfo(columns, stickyOffsets as any));
    // true is converted to 'start' by flatColumns, not useFixedInfo
    expect(result.current[0].fixStart).toBe(null);
  });
  ...
  it('is memoized', () => {
    ...
    const { result, rerender } = renderHook(() =>
      useFixedInfo(columns, stickyOffsets as any),
    );
...
describe('getCellFixedInfo', () => {
  it('returns empty object for undefined column', () => {
    const result = getCellFixedInfo(0, 0, [], { start: [], end: [], widths: [] });
    expect(result.fixStart).toBe(null);
    expect(result.fixEnd).toBe(null);
  });

  it('returns empty object for index out of bounds', () => {
    const columns = [{ fixed: 'start' }] as any;
    const result = getCellFixedInfo(5, 5, columns, { start: [0], end: [0], widths: [100] });
    expect(result.fixStart).toBe(null);
    expect(result.fixEnd).toBe(null);
  });
  ...
  it('returns start info for boolean true fixed', () => {
    const columns = [{ fixed: true }] as any;
    const result = getCellFixedInfo(0, 0, columns, { start: [0], end: [0], widths: [100] });
    expect(result.fixStart).toBe(null); // true is converted to 'start' by flatColumns
  });
});

// ============================================================
// getColumnsKey (core utils)
// ============================================================
describe('getColumnsKey (core)', () => {
  it('extracts keys from columns', () => {
    ...
    expect(keys).toEqual(['a', 'b', 'RC_TABLE_KEY']);
  });

  it('handles null/undefined columns in array', () => {
    const columns = [{ key: 'a' }, null, undefined, { key: 'b' }] as any;
    const keys = getColumnsKey(columns);
    // null and undefined columns get 'RC_TABLE_KEY' (deduplicated to 'RC_TABLE_KEY_next')
    expect(keys).toEqual(['a', 'RC_TABLE_KEY', 'RC_TABLE_KEY_next', 'b']);
  });
  ...
  it('joins array dataIndex with hyphen', () => {
